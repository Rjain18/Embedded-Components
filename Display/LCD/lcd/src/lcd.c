...
e_Status LCD_Init()
{
	...
	{
		COMMON_DELAY(45);
		returnValue = LCD_CommandWrite(LCD_FUNC_SET | LCD_8BITMODE);
		COMMON_DELAY(5);
		returnValue = LCD_CommandWrite(LCD_FUNC_SET | LCD_8BITMODE);
		COMMON_DELAY(1);
		returnValue = LCD_CommandWrite(LCD_FUNC_SET | LCD_8BITMODE);
		COMMON_DELAY(10);
		returnValue = LCD_CommandWrite(LCD_FUNC_SET | LCD_4BITMODE);
		COMMON_DELAY(10);
		returnValue = LCD_CommandWrite(LCD_FUNC_SET | LCD_4BITMODE | LCD_2LINE | LCD_5x8DOTS);
		COMMON_DELAY(10);
		displayControl = LCD_DISPLAY_CONTROL | LCD_DISPLAY_OFF | LCD_CURSOR_OFF | LCD_BLINK_OFF;
		returnValue = LCD_CommandWrite(displayControl);
		COMMON_DELAY(1);
		returnValue = LCD_CommandWrite(LCD_CLEAR_DISPLAY);
		COMMON_DELAY(5);
		returnValue = LCD_CommandWrite(LCD_ENTRY_MODE_SET | LCD_INCREMENT);
		COMMON_DELAY(1);
		displayControl = LCD_DISPLAY_CONTROL | LCD_DISPLAY_ON | LCD_CURSOR_ON | LCD_BLINK_OFF;
		returnValue = LCD_CommandWrite(displayControl);
		COMMON_DELAY(1);
	}
	else
